...
import {
  getSheetConfigFn,
  saveSheetConfigFn,
  refreshReimbursementsFn,
  probeSheetFn,
  rowsToReimbursements,
  getReimbursementCacheFn,
  type RawReimbursementRow,
} from "@/lib/reimb.functions";
import {
  CANONICAL_FIELDS,
  type ComparisonMode,
  type Config,
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Painel de reembolsos conectado à planilha online (Google Sheets ou Excel).",
      },
    ],
  }),
  component: App,
      ...
      <div className="p-3 border-t border-ep-outline-variant">
        ...
        >
          ...
          <span suppressHydrationWarning>
            {themeMounted ? (dark ? "Modo claro" : "Modo escuro") : "Tema"}
          </span>
        </button>
      </div>
    </aside>
  ...
  return (
    <header className="fixed left-64 right-0 top-0 z-20 flex h-16 items-center justify-between border-b border-ep-outline-variant bg-ep-surface-lowest/95 backdrop-blur px-6 no-print">
      <div className="flex items-center gap-3 text-sm text-ep-on-surface-variant">
        ...
        <button
          onClick={onRefresh}
          className="rounded-md border border-ep-outline-variant px-2 py-1 text-xs hover:bg-ep-surface-low"
        >
          Atualizar agora
        </button>
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={() => setDark(!dark)}
          className="grid h-9 w-9 place-items-center rounded-lg hover:bg-ep-surface-low"
        ...
        <div className="relative">
          <button
            ...
            className="relative grid h-9 w-9 place-items-center rounded-lg hover:bg-ep-surface-low"
          >
  ...
  return (
    <div className="absolute right-0 mt-2 w-80 rounded-lg border border-ep-outline-variant bg-ep-surface-lowest shadow-lg z-50">
      <div className="flex items-center justify-between border-b border-ep-outline-variant px-4 py-3 text-sm font-semibold">
        Notificações
        <button onClick={onClose} className="text-ep-on-surface-variant">
          <Icon name="close" className="text-[18px]" />
        </button>
      </div>
      {notifications.length === 0 ? (
        <div className="p-4 text-sm text-ep-on-surface-variant">
          ...
          {notifications.map((n) => (
            <li key={n.id} className="border-b border-ep-outline-variant px-4 py-3 last:border-b-0">
              <div className="flex items-start gap-2">
                <span
                  className={`mt-0.5 inline-block h-2 w-2 shrink-0 rounded-full ${n.severity === "critico" ? "bg-ep-error" : n.severity === "atencao" ? "bg-ep-tertiary" : "bg-ep-primary"}`}
                />
                <div>
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-ep-surface-lowest/85 backdrop-blur-2xl border-b border-ep-outline-variant px-4 py-3 flex justify-between items-center h-16">
      <div className="flex items-center gap-2">
        <div className="w-9 h-9 bg-ep-primary/20 border border-ep-primary/30 rounded-xl flex items-center justify-center">
          <Icon
            name="receipt_long"
            className="text-ep-primary text-[22px]"
      ...
      <button
        onClick={() => setDark(!dark)}
        className="w-10 h-10 flex items-center justify-center text-ep-on-surface-variant"
        aria-label="Trocar tema"
      >
        <Icon name="dark_mode" />
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-ep-surface-lowest/85 backdrop-blur-2xl border-t border-ep-outline-variant flex justify-around items-center h-20 pb-safe px-4">
      {items.map((n) => {
        ...
        return (
          ...
          >
            <div
              className={`flex items-center justify-center rounded-full px-6 py-2 transition-all duration-300 active:scale-90 ${active ? "bg-ep-primary/20 text-ep-primary border border-ep-primary/30" : "text-ep-on-surface-variant"}`}
            >
              <Icon
                name={n.icon}
                className="text-[26px]"
            ...
            <span
              className={`text-[10px] font-bold uppercase tracking-widest mt-1 ${active ? "text-ep-primary" : "text-ep-on-surface-variant"}`}
            >
              {n.label}
            </span>
          </button>
...
function ReembolsosView({
  ...
  const [customRange, setCustomRange] = useState<
    { current: DateRange; previous: DateRange } | undefined
  >();
  ...
  const ranges = useMemo(
    () => getComparisonRanges(comparison, customRange),
    [comparison, customRange],
  );
  ...
  const prevFiltered = useMemo(
    () => applyFilters(items, { ...filters, range: ranges.previous }),
    [items, filters, ranges],
  );
  const kpisCur = useMemo(() => computeKpis(filtered), [filtered]);
  const kpisPrev = useMemo(() => computeKpis(prevFiltered), [prevFiltered]);
  const insights = useMemo(
    () => generateInsights(items, ranges.current, ranges.previous, ranges.label),
    [items, ranges],
  );
  const monthlyBase = useMemo(
    () => applyFilters(items, { ...filters, range: undefined }),
    [items, filters],
  );
  ...
  if (!connected) {
    return (
      <EmptyState
        ...
        body="Vá em Dados & Exportação, cole o link do Google Sheets ou Excel Online e mapeie as colunas."
  ...
  if (loading && !items.length)
    return (
      <div className="grid place-items-center py-24 text-ep-on-surface-variant">
        <Icon name="hourglass" className="animate-spin" /> Carregando dados...
      </div>
    );
      ...
      <EmptyState
        ...
        body={
          lastSyncError
            ...
            : "Aguardando a primeira sincronização. Tente 'Atualizar agora' ou revise o mapeamento das colunas."
        }
      />
    );
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-4 xl:grid-cols-4">
        <KpiCard
          ...
          value={fmtBRL(kpisCur.total)}
          delta={deltaPct(kpisCur.total, kpisPrev.total)}
          note={ranges.label}
          invert
        />
        <KpiCard
          ...
          label="Pendentes"
          value={fmtBRL(kpisCur.totalPending)}
          delta={deltaPct(kpisCur.totalPending, kpisPrev.totalPending)}
          note={`${kpisCur.pendente} solicitações`}
          invert
        />
        <KpiCard
          icon="task_alt"
          label="Taxa de Pagamento"
          value={`${kpisCur.paymentRate.toFixed(1)}%`}
          delta={kpisCur.paymentRate - kpisPrev.paymentRate}
          unit="pp"
          note={`${kpisCur.realizado} realizados`}
        />
        <KpiCard
          icon="receipt"
          label="Ticket Médio"
          value={fmtBRLDec(kpisCur.avgAmount)}
          delta={deltaPct(kpisCur.avgAmount, kpisPrev.avgAmount)}
          note={`${kpisCur.count} solicitações`}
          invert
        />
      </div>
      ...
      <Card>
        <CardTitle
          ...
          subtitle={`${filtered.length} resultados · clique no colaborador para ver histórico`}
        />
      ...
      {selectedDept && (
        <DepartmentModal
          dept={selectedDept}
          items={items.filter(
            (i) => i.department === selectedDept && inRange(i.date, ranges.current),
          )}
      ...
      {selectedEmployee && (
        <EmployeeModal
          ...
          items={items.filter((i) => i.employee === selectedEmployee)}
...
function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <section
      className={`rounded-2xl border border-ep-outline-variant bg-ep-surface-lowest p-4 md:p-5 ${className}`}
    >
      {children}
    </section>
  );
  ...
  return (
    <div className="rounded-2xl border border-ep-outline-variant bg-ep-surface-lowest p-4 md:p-5">
      <div className="flex items-center justify-between">
        <span className="grid h-9 w-9 md:h-10 md:w-10 place-items-center rounded-lg bg-ep-primary/10 text-ep-primary">
          <Icon name={icon} />
        </span>
        <span
          className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] md:text-xs ${positiveDir ? "bg-ep-success/10 text-ep-success" : "bg-ep-error/10 text-ep-error"}`}
        >
          <Icon name={delta >= 0 ? "trending_up" : "trending_down"} className="text-sm" />
          {unit === "pp" ? `${delta >= 0 ? "+" : ""}${delta.toFixed(1)} pp` : fmtPct(delta)}
        </span>
      </div>
      <div className="mt-3 md:mt-4 text-[10px] md:text-xs uppercase tracking-wide text-ep-on-surface-variant">
        {label}
      </div>
  ...
  return (
    <div className="rounded-2xl border border-ep-outline-variant bg-ep-surface-lowest p-3 md:p-4">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <span className="text-[10px] md:text-xs font-semibold uppercase text-ep-on-surface-variant">
          Comparação:
        </span>
        {(
          [
          ...
          <button
            ...
            className={`rounded-full px-3 py-1 text-xs ${comparison === k ? "bg-ep-primary text-ep-on-primary" : "border border-ep-outline-variant text-ep-on-surface-variant hover:bg-ep-surface-low"}`}
          >
            {l}
          </button>
        ))}
        ...
        <div className="mb-3 grid grid-cols-2 gap-2 md:grid-cols-4">
          {(["current.from", "current.to", "previous.from", "previous.to"] as const).map((path) => {
            ...
            return (
              <label
                key={path}
                className="flex flex-col gap-1 text-[10px] md:text-xs text-ep-on-surface-variant"
              >
                {g === "current" ? "Atual" : "Anterior"} — {p === "from" ? "início" : "fim"}
                <input
                  type="date"
                  value={val ? val.toISOString().slice(0, 10) : ""}
                  onChange={(e) => {
                    const d = e.target.value ? new Date(e.target.value) : null;
                    const today = new Date();
                    const base = customRange ?? {
                      current: { from: today, to: today },
                      previous: { from: today, to: today },
                    };
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-5">
        <select
          ...
          className="rounded-md border border-ep-outline-variant bg-ep-surface-lowest px-2 py-1.5 text-sm"
        >
        ...
        <select
          ...
          onChange={(e) =>
            setFilters({
              ...filters,
              status: (e.target.value || undefined) as ReimbStatus | undefined,
            })
          }
          className="rounded-md border border-ep-outline-variant bg-ep-surface-lowest px-2 py-1.5 text-sm"
        >
...
function InsightsList({ items }: { items: Insight[] }) {
  if (!items.length)
    return (
      <div className="text-sm text-ep-on-surface-variant">Sem sinais relevantes no período.</div>
    );
  const tone = (s: Insight["severity"]) =>
    s === "critico"
      ? "border-ep-error text-ep-error"
      : s === "atencao"
        ? "border-ep-tertiary text-ep-tertiary"
        : s === "positivo"
          ? "border-ep-success text-ep-success"
  ...
  return (
    <ul className="space-y-3">
      {items.map((i) => (
        <li
          key={i.id}
          className={`rounded-lg border-l-4 bg-ep-surface-low p-3 ${tone(i.severity)}`}
        >
      ...
      {data.slice(0, 6).map((d) => (
        <li key={d.label}>
          <button
            ...
            className="w-full rounded-lg p-2 md:p-3 text-left transition hover:bg-ep-surface-low"
          >
            <div className="flex items-center justify-between text-sm">
              <span className="font-medium">{d.label}</span>
              <span className="text-ep-on-surface-variant text-xs">
                {fmtBRL(d.total)} · {d.count}
              </span>
            </div>
            <div className="mt-2 h-1.5 w-full rounded-full bg-ep-surface-highest">
              <div
                className="h-full rounded-full bg-ep-primary"
...
function StatusBadge({ status }: { status: ReimbStatus }) {
  return (
    <span
      className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${status === "realizado" ? "bg-ep-success/15 text-ep-success border border-ep-success/30" : "bg-ep-tertiary/15 text-ep-tertiary border border-ep-tertiary/30"}`}
    >
      {status}
    </span>
  );
...
function TransactionsTable({
  ...
  items: Reimbursement[];
  onSelectEmployee: (e: string) => void;
}) {
  if (!items.length)
    return (
      <div className="py-6 text-center text-sm text-ep-on-surface-variant">Nenhuma transação.</div>
    );
    ...
    <>
      <div className="md:hidden space-y-2">
        {items.map((i) => (
          <button
            key={i.id}
            onClick={() => onSelectEmployee(i.employee)}
            className="w-full flex items-center justify-between p-3 rounded-xl border border-ep-outline-variant bg-ep-surface-low active:scale-[0.99] transition"
          >
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-full bg-ep-primary/15 border border-ep-primary/20 flex items-center justify-center text-ep-primary shrink-0">
                <Icon name="person" className="text-[22px]" />
              </div>
              <div className="flex flex-col min-w-0 text-left">
                <span className="font-semibold text-sm truncate">{i.employee}</span>
                <span className="text-[11px] text-ep-on-surface-variant">
                  {i.date.toLocaleDateString("pt-BR")} · {i.category}
                </span>
              </div>
            </div>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {items.map((i) => (
              <tr
                key={i.id}
                className="border-b border-ep-outline-variant/40 last:border-b-0 hover:bg-ep-surface-low cursor-pointer"
                onClick={() => onSelectEmployee(i.employee)}
              >
                <td className="py-2 pr-3 text-ep-on-surface-variant">
                  {i.date.toLocaleDateString("pt-BR")}
                </td>
                <td className="py-2 pr-3 font-medium text-ep-primary hover:underline">
                  {i.employee}
                </td>
  ...
  return (
    <Modal onClose={onClose} title={dept} subtitle="Visão detalhada do departamento no período">
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <Card>
          <CardTitle title="Evolução mensal" />
          <MonthlyChart data={monthly} />
        </Card>
        <Card>
          <CardTitle title="Top categorias" />
          <CategoryList data={byCat} total={k.total} />
        </Card>
        <Card className="md:col-span-2">
          ...
          <ul className="space-y-2">
            {byEmployee.map((e) => (
              <li key={e.label}>
                <button
                  onClick={() => onSelectEmployee(e.label)}
                  className="w-full text-left flex items-center justify-between p-2 rounded-lg hover:bg-ep-surface-low"
                >
                  <span className="text-sm font-medium text-ep-primary">{e.label || "—"}</span>
                  <span className="text-xs text-ep-on-surface-variant">
                    {fmtBRL(e.total)} · {e.count}
                  </span>
                </button>
              </li>
            ))}
...
function EmployeeModal({
  ...
  return (
    <Modal
      onClose={onClose}
      title={employee}
      subtitle={`${dept} · ${items.length} registro(s) no total`}
    >
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Stat
          label="Status atual"
          value={lastStatus ? lastStatus[0].toUpperCase() + lastStatus.slice(1) : "—"}
        />
      </div>
        ...
        <Card>
          ...
          <div className="space-y-2 max-h-[50vh] overflow-y-auto custom-scrollbar pr-1">
            {sorted.map((i) => (
              <div
                key={i.id}
                className="rounded-lg border border-ep-outline-variant bg-ep-surface-low p-3"
              >
                ...
                {(i.description || i.observacao) && (
                  <div className="mt-2 pt-2 border-t border-ep-outline-variant/60 text-xs space-y-1">
                    {i.description && (
                      <p>
                        <span className="font-semibold text-ep-on-surface-variant">Descrição:</span>{" "}
                        {i.description}
                      </p>
                    )}
                    {i.observacao && (
                      <p>
                        <span className="font-semibold text-ep-on-surface-variant">
                          Observação:
                        </span>{" "}
                        {i.observacao}
                      </p>
            ...
            {!sorted.length && (
              <div className="text-sm text-ep-on-surface-variant">Sem registros.</div>
            )}
          </div>
        </Card>
      </div>
...
function Modal({
  ...
  return (
    ...
    >
      <div
        className="w-full md:max-w-4xl max-h-[92vh] overflow-y-auto rounded-t-2xl md:rounded-2xl bg-ep-surface-lowest p-4 md:p-6 custom-scrollbar border-t md:border border-ep-outline-variant pb-safe"
      ...
      >
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="grid h-8 w-8 place-items-center rounded-lg hover:bg-ep-surface-low"
          >
            <Icon name="close" />
          </button>
        </div>
...
function EmptyState({
  ...
  return (
    <div className="grid place-items-center rounded-2xl border border-dashed border-ep-outline-variant bg-ep-surface-lowest p-8 md:p-12 text-center">
      <span className="grid h-12 w-12 place-items-center rounded-full bg-ep-surface-low text-ep-primary">
        <Icon name={icon} />
      </span>
      ...
      {cta && (
        <button
          onClick={cta.onClick}
          className="mt-4 rounded-md bg-ep-primary px-4 py-2 text-sm text-ep-on-primary"
        >
          {cta.label}
        </button>
      )}
...
function ExportView({
  ...
  const detectMeta = async () => {
    ...
    try {
      ...
      const validSheet = m.sheets.some((s) => s.title === sheet)
        ? sheet
        : (m.sheets[0]?.title ?? "");
      setSheet(validSheet);
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <Card>
        <CardTitle
          title="Conectar planilha online"
          subtitle="Cole o link de uma planilha Google Sheets ou Excel Online (SharePoint/OneDrive). Os dados ficam armazenados no banco e atualizam a cada 5 min em todos os dispositivos."
        />
        <div className="flex flex-col gap-2 md:flex-row">
          <input
            value={url}
            onChange={(e) => setUrl(e.target.value)}
            placeholder="https://docs.google.com/spreadsheets/... ou https://...sharepoint.com/..."
            className="flex-1 rounded-md border border-ep-outline-variant bg-ep-surface-lowest px-3 py-2 text-sm"
          />
          <button
            onClick={detectMeta}
            disabled={!url || loading}
            className="rounded-md bg-ep-primary px-4 py-2 text-sm text-ep-on-primary disabled:opacity-50"
          >
        ...
        {config.lastSyncAt && (
          <div className="mt-2 text-xs text-ep-on-surface-variant">
            ...
            {config.lastSyncError && (
              <span className="ml-2 text-ep-error">· erro: {config.lastSyncError}</span>
            )}
          </div>
        )}
      </Card>
      ...
      {meta && (
        <Card>
          <CardTitle
            ...
            subtitle="Diga ao sistema qual coluna da planilha representa cada campo. Status deve conter 'Pendente' ou 'Realizado'."
          />
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            {CANONICAL_FIELDS.map((f) => (
              <div
                key={f.key}
                className="flex items-center justify-between gap-2 rounded-lg border border-ep-outline-variant bg-ep-surface-low p-3"
              >
                <div>
                  <div className="text-sm font-medium">
                    {f.label} {f.required && <span className="text-ep-error">*</span>}
                  </div>
                  <div className="text-xs text-ep-on-surface-variant">
                    campo do sistema: {f.key}
                  </div>
                </div>
                <select
                  value={mapping[f.key] ?? ""}
          ...
          <div className="mt-4 flex flex-wrap items-center justify-end gap-2">
            <button
              ...
              className="rounded-md border border-ep-outline-variant px-4 py-2 text-sm disabled:opacity-50"
            >
          ...
          {!canSave && (
            <p className="mt-2 text-xs text-ep-error">
              Mapeie ao menos: Data, Valor, Departamento, Colaborador, Categoria e Status.
            </p>
          )}
      ...
      <Card>
        <CardTitle
          ...
          subtitle={`${items.length} registros disponíveis no cache.`}
        />
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className="rounded-md border border-ep-outline-variant px-4 py-2 text-sm"
          >
            Imprimir / PDF
          </button>
        </div>
